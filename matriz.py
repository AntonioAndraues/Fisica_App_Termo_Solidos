...
def main():
    ...
    txt = ["*COORDINATES", "*ELEMENT_GROUPS", "*INCIDENCES",
           '*GEOMETRIC_PROPERTIES', '*LOADS', '*MATERIALS', '*BCNODES']
    ...
    def check(txt):
        ...
        # print(lista)
        loads(txt_loads)
        # nessa linha quantidade de elementos barra é dado como return para posterior uso
        listabarras = elementos_barra(
            txt_barra, txt_incidencias, txt_area, txt_materials)
        ...
        for linha in range(1, int(quantidade_bcnodes)+1):
            ...
            for i in range(len(lista2)):
                if (lista2[i][0] == entrada[indice_bcnodes+linha+1].split()[0]):
                    ...
                    lista2[i].append(
                        entrada[indice_bcnodes+linha+1].split()[1])
    ...
    def loads(indice_loads):
        ...
        for linha in range(1, int(quantidade_load)+1):
            ...
            if(int(split[1]) == 2):
                ...
                temp = 1
            dictloads[entrada[indice_loads+linha+1][0]
                      ] = [lista_loads[0], lista_loads[1]]
            if(temp):
                lista_loads = [0, 0]
